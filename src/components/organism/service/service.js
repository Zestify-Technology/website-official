...
export default function ServicesSection() {
  ...
  return (
    <Section id="service" className="px-6 lg:px-12 py-24 lg:py-32">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-8 mb-16">
          ...
          <div className="lg:max-w-xs lg:text-right">
            <Paragraph align="right">
              Kami merancang implementasi AI yang bukan sekadar tren — tapi benar-benar menggerakkan pertumbuhan operasional bisnis Anda secara terukur.
            </Paragraph>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
          {services.map((service) => (
            <div
              ...
              className={`relative rounded-2xl border p-6 cursor-pointer transition-all duration-500 group overflow-hidden backdrop-blur-sm
                ${service.featured 
                  ? `bg-gradient-to-br ${service.accent} border-transparent shadow-2xl ${service.glow}` 
            ...
            >
              ...
              <div className="relative z-10 flex flex-col h-full">
                ...
                <span className={`text-xs tracking-widest font-semibold mb-2 uppercase ${service.featured ? 'text-white/60' : 'text-white/30'}`}>
                  {service.tag}
                </span>

                <H3 align="left" className="leading-snug mb-3 transition-colors duration-300 group-hover:text-white">
                  {service.title}
                </H3>

                <Paragraph fontSize="sm" className={`flex-1 transition-colors duration-300 ${service.featured ? 'text-white/70' : 'text-white/40 group-hover:text-white/60'}`}>
                  {service.desc}
                </Paragraph>
