...
export const MessageFormatter = {
  ...
  parseTables: (text) => {
    ...
    const flushTable = () => {
      ...
      html += `<thead><tr>`;
      header.forEach(h => {
        html += `<th style="padding:8px 12px;background:#ffffff;color:#000000;font-weight:600;white-space:nowrap;border:1px solid rgba(99,102,241,0.4);">${h}</th>`;
      });
      html += `</tr></thead>`;
      ...
      rows.forEach((r, rowIdx) => {
        ...
        r.forEach(c => {
          html += `<td style="padding:7px 12px;border:1px solid rgba(255,255,255,0.08);white-space:wrap;">${c}</td>`;
        });
        html += `</tr>`;
      });
